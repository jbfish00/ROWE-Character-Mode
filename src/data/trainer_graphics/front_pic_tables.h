const struct MonCoords gTrainerFrontPicCoords[] =
{
    [TRAINER_PIC_HIKER] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_AQUA_GRUNT_M] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_POKEMON_BREEDER_F] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_COOLTRAINER_M] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_BIRD_KEEPER] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_COLLECTOR] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_AQUA_GRUNT_F] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_SWIMMER_M] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_MAGMA_GRUNT_M] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_EXPERT_M] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_AQUA_ADMIN_M] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_BLACK_BELT] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_AQUA_ADMIN_F] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_AQUA_LEADER_ARCHIE] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_HEX_MANIAC] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_AROMA_LADY] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_RUIN_MANIAC] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_INTERVIEWER] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_TUBER_F] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_TUBER_M] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_COOLTRAINER_F] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LADY] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_BEAUTY] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_RICH_BOY] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_EXPERT_F] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_POKEMANIAC] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_MAGMA_GRUNT_F] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_GUITARIST] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_KINDLER] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_CAMPER] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_PICNICKER] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_BUG_MANIAC] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_POKEMON_BREEDER_M] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_PSYCHIC_M] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_PSYCHIC_F] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_GENTLEMAN] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_ELITE_FOUR_SIDNEY] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_ELITE_FOUR_PHOEBE] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_ELITE_FOUR_GLACIA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_ELITE_FOUR_DRAKE] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LEADER_ROXANNE] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LEADER_BRAWLY] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LEADER_WATTSON] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LEADER_FLANNERY] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LEADER_NORMAN] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LEADER_WINONA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LEADER_TATE_AND_LIZA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LEADER_JUAN] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_SCHOOL_KID_M] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_SCHOOL_KID_F] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_SR_AND_JR] = {.size = 8, .y_offset = 2},
    [TRAINER_PIC_POKEFAN_M] = {.size = 8, .y_offset = 2},
    [TRAINER_PIC_POKEFAN_F] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_YOUNGSTER] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_CHAMPION_WALLACE] = {.size = 8, .y_offset = 2},
    [TRAINER_PIC_FISHERMAN] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_CYCLING_TRIATHLETE_M] = {.size = 8, .y_offset = 2},
    [TRAINER_PIC_CYCLING_TRIATHLETE_F] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_RUNNING_TRIATHLETE_M] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_RUNNING_TRIATHLETE_F] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_SWIMMING_TRIATHLETE_M] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_SWIMMING_TRIATHLETE_F] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_DRAGON_TAMER] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_NINJA_BOY] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_BATTLE_GIRL] = {.size = 8, .y_offset = 2},
    [TRAINER_PIC_PARASOL_LADY] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_SWIMMER_F] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_TWINS] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_SAILOR] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_MAGMA_ADMIN] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_WALLY] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_BRENDAN] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_MAY] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_BUG_CATCHER] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_POKEMON_RANGER_M] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_POKEMON_RANGER_F] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_MAGMA_LEADER_MAXIE] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LASS] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_YOUNG_COUPLE] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_OLD_COUPLE] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_SIS_AND_BRO] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_STEVEN] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_SALON_MAIDEN_ANABEL] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_DOME_ACE_TUCKER] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_PALACE_MAVEN_SPENSER] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_ARENA_TYCOON_GRETA] = {.size = 8, .y_offset = 2},
    [TRAINER_PIC_FACTORY_HEAD_NOLAND] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_PIKE_QUEEN_LUCY] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_PYRAMID_KING_BRANDON] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_RED] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LEAF] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_RS_BRENDAN] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_RS_MAY] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_CRUSH_KIN] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_NURSE] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_HILBERT] = {.size = 8, .y_offset = 1},
// CHARMODE-SPRITES-START front-coords (generated by tools/character_mode/import_sprites.py)
	[TRAINER_PIC_BLUE] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_LANCE] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_LORELEI] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_BRUNO] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_AGATHA] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_KOGA] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_BROCK] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_MISTY] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_LT_SURGE] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_ERIKA] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_SABRINA] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_BLAINE] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_GIOVANNI] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_LUCAS] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_DAWN] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_BARRY] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_CYNTHIA] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_ROARK] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_GARDENIA] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_MAYLENE] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_CRASHER_WAKE] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_FANTINA] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_BYRON] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_CANDICE] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_VOLKNER] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_BERTHA] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_LUCIAN] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_CYRUS] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_MARS] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_JUPITER] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_SATURN] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_ETHAN] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_KRIS] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_FALKNER] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_BUGSY] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_WHITNEY] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_MORTY] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_CHUCK] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_JASMINE] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_PRYCE] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_CLAIR] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_JANINE] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_WILL] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_KAREN] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_ARCHER] = {.size = 8, .y_offset = 1},
	[TRAINER_PIC_ARIANA] = {.size = 8, .y_offset = 1},
// CHARMODE-SPRITES-END front-coords
// ROWENEWTRN-START coords (generated by tools/character_mode/port_2x_new_trainers.py)
    [TRAINER_PIC_HILDA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_PAINTER] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_ROCKET_GRUNT_M] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_ROCKET_GRUNT_F] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_SCIENTIST] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_JUGGLER] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_COOL_COUPLE] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_TAMER] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_CRUSH_GIRL] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_POKEMON_BREEDER] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LEADER_BROCK] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LEADER_MISTY] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LEADER_LT_SURGE] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LEADER_ERIKA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LEADER_KOGA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LEADER_BLAINE] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LEADER_SABRINA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LEADER_GIOVANNI] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_ELITE_FOUR_LORELEI] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_ELITE_FOUR_BRUNO] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_ELITE_FOUR_AGATHA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_ELITE_FOUR_LANCE] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_CHAMPION_RIVAL] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LEADER_FALKNER] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LEADER_BUGSY] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LEADER_WHITNEY] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LEADER_MORTY] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LEADER_CHUCK] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LEADER_JASMINE] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LEADER_PRYCE] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LEADER_CLAIR] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LEADER_ROARK] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LEADER_GARDENIA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LEADER_MAYLENE] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LEADER_CRASHER_WAKE] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LEADER_FANTINA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LEADER_BYRON] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LEADER_CANDICE] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LEADER_VOLKNER] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_RED_RBY] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_GOLD] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_ORAS_BRENDAN] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_ORAS_MAY] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_GREEN] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_DAWN_PT] = {.size = 8, .y_offset = 1},
// ROWENEWTRN-END coords
// CHARMODE-SPRITES-START donor-front-coords (generated by tools/character_mode/import_sprites.py)
    [TRAINER_PIC_GARY] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_RITCHIE] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_TRACEY] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_JESSIE] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_JAMES] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LYRA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_SILVER] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LIZA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_DREW] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_AARON] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_FLINT] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_NATE] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_ROSA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_ALDER] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_IRIS] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_SHAUNTAL] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_MARSHAL] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_GRIMSLEY] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_CAITLIN] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_CILAN] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_CHILI] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_CRESS] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LENORA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_BURGH] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_ELESA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_CLAY] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_SKYLA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_BRYCEN] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_DRAYDEN] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_CHEREN] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_ROXIE] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_MARLON] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_BIANCA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_HUGH] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_N] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_GHETSIS] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_COLRESS] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_CALEM] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_SERENA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_DIANTHA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_MALVA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_SIEBOLD] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_WIKSTROM] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_DRASNA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_VIOLA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_GRANT] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_KORRINA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_RAMOS] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_CLEMONT] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_VALERIE] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_OLYMPIA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_WULFRIC] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_SHAUNA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LYSANDRE] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_ELIO] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_SELENE] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_KUKUI] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_HAU] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_MOLAYNE] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_KAHILI] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_ACEROLA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_HALA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_OLIVIA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_NANU] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_HAPU] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_GLADION] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LILLIE] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_KIAWE] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LANA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_MALLOW] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_SOPHOCLES] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LEON] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_MILO] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_NESSA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_KABU] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_BEA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_ALLISTER] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_OPAL] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_GORDIE] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_MELONY] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_PIERS] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_RAIHAN] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_HOP] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_BEDE] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_MARNIE] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_GEETA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_NEMONA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_RIKA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_POPPY] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_HASSEL] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_KATY] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_BRASSIUS] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_IONO] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_KOFU] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_LARRY] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_RYME] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_TULIP] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_GRUSHA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_ARVEN] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_PENNY] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_VICTOR] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_GLORIA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_OAK] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_ARGENTA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_DAHLIA] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_DARACH] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_PALMER] = {.size = 8, .y_offset = 1},
    [TRAINER_PIC_THORTON] = {.size = 8, .y_offset = 1},
// CHARMODE-SPRITES-END donor-front-coords
};

const struct CompressedSpriteSheet gTrainerFrontPicTable[] =
{
    TRAINER_SPRITE(HIKER, gTrainerFrontPic_Hiker, 0x800),
    TRAINER_SPRITE(AQUA_GRUNT_M, gTrainerFrontPic_AquaGruntM, 0x800),
    TRAINER_SPRITE(POKEMON_BREEDER_F, gTrainerFrontPic_PokemonBreederF, 0x800),
    TRAINER_SPRITE(COOLTRAINER_M, gTrainerFrontPic_CoolTrainerM, 0x800),
    TRAINER_SPRITE(BIRD_KEEPER, gTrainerFrontPic_BirdKeeper, 0x800),
    TRAINER_SPRITE(COLLECTOR, gTrainerFrontPic_Collector, 0x800),
    TRAINER_SPRITE(AQUA_GRUNT_F, gTrainerFrontPic_AquaGruntF, 0x800),
    TRAINER_SPRITE(SWIMMER_M, gTrainerFrontPic_SwimmerM, 0x800),
    TRAINER_SPRITE(MAGMA_GRUNT_M, gTrainerFrontPic_MagmaGruntM, 0x800),
    TRAINER_SPRITE(EXPERT_M, gTrainerFrontPic_ExpertM, 0x800),
    TRAINER_SPRITE(AQUA_ADMIN_M, gTrainerFrontPic_AquaAdminM, 0x800),
    TRAINER_SPRITE(BLACK_BELT, gTrainerFrontPic_BlackBelt, 0x800),
    TRAINER_SPRITE(AQUA_ADMIN_F, gTrainerFrontPic_AquaAdminF, 0x800),
    TRAINER_SPRITE(AQUA_LEADER_ARCHIE, gTrainerFrontPic_AquaLeaderArchie, 0x800),
    TRAINER_SPRITE(HEX_MANIAC, gTrainerFrontPic_HexManiac, 0x800),
    TRAINER_SPRITE(AROMA_LADY, gTrainerFrontPic_AromaLady, 0x800),
    TRAINER_SPRITE(RUIN_MANIAC, gTrainerFrontPic_RuinManiac, 0x800),
    TRAINER_SPRITE(INTERVIEWER, gTrainerFrontPic_Interviewer, 0x800),
    TRAINER_SPRITE(TUBER_F, gTrainerFrontPic_TuberF, 0x800),
    TRAINER_SPRITE(TUBER_M, gTrainerFrontPic_TuberM, 0x800),
    TRAINER_SPRITE(COOLTRAINER_F, gTrainerFrontPic_CoolTrainerF, 0x800),
    TRAINER_SPRITE(LADY, gTrainerFrontPic_Lady, 0x800),
    TRAINER_SPRITE(BEAUTY, gTrainerFrontPic_Beauty, 0x800),
    TRAINER_SPRITE(RICH_BOY, gTrainerFrontPic_RichBoy, 0x800),
    TRAINER_SPRITE(EXPERT_F, gTrainerFrontPic_ExpertF, 0x800),
    TRAINER_SPRITE(POKEMANIAC, gTrainerFrontPic_Pokemaniac, 0x800),
    TRAINER_SPRITE(MAGMA_GRUNT_F, gTrainerFrontPic_MagmaGruntF, 0x800),
    TRAINER_SPRITE(GUITARIST, gTrainerFrontPic_Guitarist, 0x800),
    TRAINER_SPRITE(KINDLER, gTrainerFrontPic_Kindler, 0x800),
    TRAINER_SPRITE(CAMPER, gTrainerFrontPic_Camper, 0x800),
    TRAINER_SPRITE(PICNICKER, gTrainerFrontPic_Picnicker, 0x800),
    TRAINER_SPRITE(BUG_MANIAC, gTrainerFrontPic_BugManiac, 0x800),
    TRAINER_SPRITE(POKEMON_BREEDER_M, gTrainerFrontPic_PokemonBreederM, 0x800),
    TRAINER_SPRITE(PSYCHIC_M, gTrainerFrontPic_PsychicM, 0x800),
    TRAINER_SPRITE(PSYCHIC_F, gTrainerFrontPic_PsychicF, 0x800),
    TRAINER_SPRITE(GENTLEMAN, gTrainerFrontPic_Gentleman, 0x800),
    TRAINER_SPRITE(ELITE_FOUR_SIDNEY, gTrainerFrontPic_EliteFourSidney, 0x800),
    TRAINER_SPRITE(ELITE_FOUR_PHOEBE, gTrainerFrontPic_EliteFourPhoebe, 0x800),
    TRAINER_SPRITE(ELITE_FOUR_GLACIA, gTrainerFrontPic_EliteFourGlacia, 0x800),
    TRAINER_SPRITE(ELITE_FOUR_DRAKE, gTrainerFrontPic_EliteFourDrake, 0x800),
    TRAINER_SPRITE(LEADER_ROXANNE, gTrainerFrontPic_LeaderRoxanne, 0x800),
    TRAINER_SPRITE(LEADER_BRAWLY, gTrainerFrontPic_LeaderBrawly, 0x800),
    TRAINER_SPRITE(LEADER_WATTSON, gTrainerFrontPic_LeaderWattson, 0x800),
    TRAINER_SPRITE(LEADER_FLANNERY, gTrainerFrontPic_LeaderFlannery, 0x800),
    TRAINER_SPRITE(LEADER_NORMAN, gTrainerFrontPic_LeaderNorman, 0x800),
    TRAINER_SPRITE(LEADER_WINONA, gTrainerFrontPic_LeaderWinona, 0x800),
    TRAINER_SPRITE(LEADER_TATE_AND_LIZA, gTrainerFrontPic_LeaderTateAndLiza, 0x800),
    TRAINER_SPRITE(LEADER_JUAN, gTrainerFrontPic_LeaderJuan, 0x800),
    TRAINER_SPRITE(SCHOOL_KID_M, gTrainerFrontPic_SchoolKidM, 0x800),
    TRAINER_SPRITE(SCHOOL_KID_F, gTrainerFrontPic_SchoolKidF, 0x800),
    TRAINER_SPRITE(SR_AND_JR, gTrainerFrontPic_SrAndJr, 0x1000),
    TRAINER_SPRITE(POKEFAN_M, gTrainerFrontPic_PokefanM, 0x1000),
    TRAINER_SPRITE(POKEFAN_F, gTrainerFrontPic_PokefanF, 0x800),
    TRAINER_SPRITE(YOUNGSTER, gTrainerFrontPic_Youngster, 0x800),
    TRAINER_SPRITE(CHAMPION_WALLACE, gTrainerFrontPic_ChampionWallace, 0x1000),
    TRAINER_SPRITE(FISHERMAN, gTrainerFrontPic_Fisherman, 0x800),
    TRAINER_SPRITE(CYCLING_TRIATHLETE_M, gTrainerFrontPic_CyclingTriathleteM, 0x1000),
    TRAINER_SPRITE(CYCLING_TRIATHLETE_F, gTrainerFrontPic_CyclingTriathleteF, 0x800),
    TRAINER_SPRITE(RUNNING_TRIATHLETE_M, gTrainerFrontPic_RunningTriathleteM, 0x800),
    TRAINER_SPRITE(RUNNING_TRIATHLETE_F, gTrainerFrontPic_RunningTriathleteF, 0x800),
    TRAINER_SPRITE(SWIMMING_TRIATHLETE_M, gTrainerFrontPic_SwimmingTriathleteM, 0x800),
    TRAINER_SPRITE(SWIMMING_TRIATHLETE_F, gTrainerFrontPic_SwimmingTriathleteF, 0x800),
    TRAINER_SPRITE(DRAGON_TAMER, gTrainerFrontPic_DragonTamer, 0x800),
    TRAINER_SPRITE(NINJA_BOY, gTrainerFrontPic_NinjaBoy, 0x800),
    TRAINER_SPRITE(BATTLE_GIRL, gTrainerFrontPic_BattleGirl, 0x1000),
    TRAINER_SPRITE(PARASOL_LADY, gTrainerFrontPic_ParasolLady, 0x800),
    TRAINER_SPRITE(SWIMMER_F, gTrainerFrontPic_SwimmerF, 0x800),
    TRAINER_SPRITE(TWINS, gTrainerFrontPic_Twins, 0x800),
    TRAINER_SPRITE(SAILOR, gTrainerFrontPic_Sailor, 0x800),
    TRAINER_SPRITE(MAGMA_ADMIN, gTrainerFrontPic_MagmaAdmin, 0x800),
    TRAINER_SPRITE(WALLY, gTrainerFrontPic_Wally, 0x800),
    TRAINER_SPRITE(BRENDAN, gTrainerFrontPic_Brendan, 0x800),
    TRAINER_SPRITE(MAY, gTrainerFrontPic_May, 0x800),
    TRAINER_SPRITE(BUG_CATCHER, gTrainerFrontPic_BugCatcher, 0x800),
    TRAINER_SPRITE(POKEMON_RANGER_M, gTrainerFrontPic_PokemonRangerM, 0x800),
    TRAINER_SPRITE(POKEMON_RANGER_F, gTrainerFrontPic_PokemonRangerF, 0x800),
    TRAINER_SPRITE(MAGMA_LEADER_MAXIE, gTrainerFrontPic_MagmaLeaderMaxie, 0x800),
    TRAINER_SPRITE(LASS, gTrainerFrontPic_Lass, 0x800),
    TRAINER_SPRITE(YOUNG_COUPLE, gTrainerFrontPic_YoungCouple, 0x800),
    TRAINER_SPRITE(OLD_COUPLE, gTrainerFrontPic_OldCouple, 0x800),
    TRAINER_SPRITE(SIS_AND_BRO, gTrainerFrontPic_SisAndBro, 0x800),
    TRAINER_SPRITE(STEVEN, gTrainerFrontPic_Steven, 0x800),
    TRAINER_SPRITE(SALON_MAIDEN_ANABEL, gTrainerFrontPic_SalonMaidenAnabel, 0x800),
    TRAINER_SPRITE(DOME_ACE_TUCKER, gTrainerFrontPic_DomeAceTucker, 0x800),
    TRAINER_SPRITE(PALACE_MAVEN_SPENSER, gTrainerFrontPic_PalaceMavenSpenser, 0x800),
    TRAINER_SPRITE(ARENA_TYCOON_GRETA, gTrainerFrontPic_ArenaTycoonGreta, 0x1000),
    TRAINER_SPRITE(FACTORY_HEAD_NOLAND, gTrainerFrontPic_FactoryHeadNoland, 0x800),
    TRAINER_SPRITE(PIKE_QUEEN_LUCY, gTrainerFrontPic_PikeQueenLucy, 0x800),
    TRAINER_SPRITE(PYRAMID_KING_BRANDON, gTrainerFrontPic_PyramidKingBrandon, 0x800),
    TRAINER_SPRITE(RED, gTrainerFrontPic_Red, 0x800),
    TRAINER_SPRITE(LEAF, gTrainerFrontPic_Leaf, 0x800),
    TRAINER_SPRITE(RS_BRENDAN, gTrainerFrontPic_RubySapphireBrendan, 0x800),
    TRAINER_SPRITE(RS_MAY, gTrainerFrontPic_RubySapphireMay, 0x800),
	TRAINER_SPRITE(CRUSH_KIN, gTrainerFrontPic_CrushKin, 0x800),
	TRAINER_SPRITE(NURSE, gTrainerFrontPic_BlueNurse, 0x800),
	TRAINER_SPRITE(HILBERT, gTrainerFrontPic_Hilbert, 0x800),
// CHARMODE-SPRITES-START front-sprites (generated by tools/character_mode/import_sprites.py)
	TRAINER_SPRITE(BLUE, gTrainerFrontPic_CmBlue, 0x800),
	TRAINER_SPRITE(LANCE, gTrainerFrontPic_CmLance, 0x800),
	TRAINER_SPRITE(LORELEI, gTrainerFrontPic_CmLorelei, 0x800),
	TRAINER_SPRITE(BRUNO, gTrainerFrontPic_CmBruno, 0x800),
	TRAINER_SPRITE(AGATHA, gTrainerFrontPic_CmAgatha, 0x800),
	TRAINER_SPRITE(KOGA, gTrainerFrontPic_CmKoga, 0x800),
	TRAINER_SPRITE(BROCK, gTrainerFrontPic_CmBrock, 0x800),
	TRAINER_SPRITE(MISTY, gTrainerFrontPic_CmMisty, 0x800),
	TRAINER_SPRITE(LT_SURGE, gTrainerFrontPic_CmLtSurge, 0x800),
	TRAINER_SPRITE(ERIKA, gTrainerFrontPic_CmErika, 0x800),
	TRAINER_SPRITE(SABRINA, gTrainerFrontPic_CmSabrina, 0x800),
	TRAINER_SPRITE(BLAINE, gTrainerFrontPic_CmBlaine, 0x800),
	TRAINER_SPRITE(GIOVANNI, gTrainerFrontPic_CmGiovanni, 0x800),
	TRAINER_SPRITE(LUCAS, gTrainerFrontPic_CmLucas, 0x800),
	TRAINER_SPRITE(DAWN, gTrainerFrontPic_CmDawn, 0x800),
	TRAINER_SPRITE(BARRY, gTrainerFrontPic_CmBarry, 0x800),
	TRAINER_SPRITE(CYNTHIA, gTrainerFrontPic_CmCynthia, 0x800),
	TRAINER_SPRITE(ROARK, gTrainerFrontPic_CmRoark, 0x800),
	TRAINER_SPRITE(GARDENIA, gTrainerFrontPic_CmGardenia, 0x800),
	TRAINER_SPRITE(MAYLENE, gTrainerFrontPic_CmMaylene, 0x800),
	TRAINER_SPRITE(CRASHER_WAKE, gTrainerFrontPic_CmCrasherWake, 0x800),
	TRAINER_SPRITE(FANTINA, gTrainerFrontPic_CmFantina, 0x800),
	TRAINER_SPRITE(BYRON, gTrainerFrontPic_CmByron, 0x800),
	TRAINER_SPRITE(CANDICE, gTrainerFrontPic_CmCandice, 0x800),
	TRAINER_SPRITE(VOLKNER, gTrainerFrontPic_CmVolkner, 0x800),
	TRAINER_SPRITE(BERTHA, gTrainerFrontPic_CmBertha, 0x800),
	TRAINER_SPRITE(LUCIAN, gTrainerFrontPic_CmLucian, 0x800),
	TRAINER_SPRITE(CYRUS, gTrainerFrontPic_CmCyrus, 0x800),
	TRAINER_SPRITE(MARS, gTrainerFrontPic_CmMars, 0x800),
	TRAINER_SPRITE(JUPITER, gTrainerFrontPic_CmJupiter, 0x800),
	TRAINER_SPRITE(SATURN, gTrainerFrontPic_CmSaturn, 0x800),
	TRAINER_SPRITE(ETHAN, gTrainerFrontPic_CmEthan, 0x800),
	TRAINER_SPRITE(KRIS, gTrainerFrontPic_CmKris, 0x800),
	TRAINER_SPRITE(FALKNER, gTrainerFrontPic_CmFalkner, 0x800),
	TRAINER_SPRITE(BUGSY, gTrainerFrontPic_CmBugsy, 0x800),
	TRAINER_SPRITE(WHITNEY, gTrainerFrontPic_CmWhitney, 0x800),
	TRAINER_SPRITE(MORTY, gTrainerFrontPic_CmMorty, 0x800),
	TRAINER_SPRITE(CHUCK, gTrainerFrontPic_CmChuck, 0x800),
	TRAINER_SPRITE(JASMINE, gTrainerFrontPic_CmJasmine, 0x800),
	TRAINER_SPRITE(PRYCE, gTrainerFrontPic_CmPryce, 0x800),
	TRAINER_SPRITE(CLAIR, gTrainerFrontPic_CmClair, 0x800),
	TRAINER_SPRITE(JANINE, gTrainerFrontPic_CmJanine, 0x800),
	TRAINER_SPRITE(WILL, gTrainerFrontPic_CmWill, 0x800),
	TRAINER_SPRITE(KAREN, gTrainerFrontPic_CmKaren, 0x800),
	TRAINER_SPRITE(ARCHER, gTrainerFrontPic_CmArcher, 0x800),
	TRAINER_SPRITE(ARIANA, gTrainerFrontPic_CmAriana, 0x800),
// CHARMODE-SPRITES-END front-sprites
// ROWENEWTRN-START fronttbl (generated by tools/character_mode/port_2x_new_trainers.py)
    TRAINER_SPRITE(HILDA, gTrainerFrontPic_Hilda, 0x800),
    TRAINER_SPRITE(PAINTER, gTrainerFrontPic_Painter, 0x800),
    TRAINER_SPRITE(ROCKET_GRUNT_M, gTrainerFrontPic_Rocket_Grunt_M, 0x800),
    TRAINER_SPRITE(ROCKET_GRUNT_F, gTrainerFrontPic_Rocket_Grunt_F, 0x800),
    TRAINER_SPRITE(SCIENTIST, gTrainerFrontPic_Scientist, 0x800),
    TRAINER_SPRITE(JUGGLER, gTrainerFrontPic_Juggler, 0x800),
    TRAINER_SPRITE(COOL_COUPLE, gTrainerFrontPic_Cool_Couple, 0x800),
    TRAINER_SPRITE(TAMER, gTrainerFrontPic_Tamer, 0x800),
    TRAINER_SPRITE(CRUSH_GIRL, gTrainerFrontPic_Crush_Girl, 0x800),
    TRAINER_SPRITE(POKEMON_BREEDER, gTrainerFrontPic_Pokemon_Breeder, 0x800),
    TRAINER_SPRITE(LEADER_BROCK, gTrainerFrontPic_Leader_Brock, 0x800),
    TRAINER_SPRITE(LEADER_MISTY, gTrainerFrontPic_Leader_Misty, 0x800),
    TRAINER_SPRITE(LEADER_LT_SURGE, gTrainerFrontPic_Leader_Lt_Surge, 0x800),
    TRAINER_SPRITE(LEADER_ERIKA, gTrainerFrontPic_Leader_Erika, 0x800),
    TRAINER_SPRITE(LEADER_KOGA, gTrainerFrontPic_Leader_Koga, 0x800),
    TRAINER_SPRITE(LEADER_BLAINE, gTrainerFrontPic_Leader_Blaine, 0x800),
    TRAINER_SPRITE(LEADER_SABRINA, gTrainerFrontPic_Leader_Sabrina, 0x800),
    TRAINER_SPRITE(LEADER_GIOVANNI, gTrainerFrontPic_Leader_Giovanni, 0x800),
    TRAINER_SPRITE(ELITE_FOUR_LORELEI, gTrainerFrontPic_Elite_Four_Lorelei, 0x800),
    TRAINER_SPRITE(ELITE_FOUR_BRUNO, gTrainerFrontPic_Elite_Four_Bruno, 0x800),
    TRAINER_SPRITE(ELITE_FOUR_AGATHA, gTrainerFrontPic_Elite_Four_Agatha, 0x800),
    TRAINER_SPRITE(ELITE_FOUR_LANCE, gTrainerFrontPic_Elite_Four_Lance, 0x800),
    TRAINER_SPRITE(CHAMPION_RIVAL, gTrainerFrontPic_Champion_Rival, 0x800),
    TRAINER_SPRITE(LEADER_FALKNER, gTrainerFrontPic_Leader_Falkner, 0x800),
    TRAINER_SPRITE(LEADER_BUGSY, gTrainerFrontPic_Leader_Bugsy, 0x800),
    TRAINER_SPRITE(LEADER_WHITNEY, gTrainerFrontPic_Leader_Whitney, 0x800),
    TRAINER_SPRITE(LEADER_MORTY, gTrainerFrontPic_Leader_Morty, 0x800),
    TRAINER_SPRITE(LEADER_CHUCK, gTrainerFrontPic_Leader_Chuck, 0x800),
    TRAINER_SPRITE(LEADER_JASMINE, gTrainerFrontPic_Leader_Jasmine, 0x800),
    TRAINER_SPRITE(LEADER_PRYCE, gTrainerFrontPic_Leader_Pryce, 0x800),
    TRAINER_SPRITE(LEADER_CLAIR, gTrainerFrontPic_Leader_Clair, 0x800),
    TRAINER_SPRITE(LEADER_ROARK, gTrainerFrontPic_Leader_Roark, 0x800),
    TRAINER_SPRITE(LEADER_GARDENIA, gTrainerFrontPic_Leader_Gardenia, 0x800),
    TRAINER_SPRITE(LEADER_MAYLENE, gTrainerFrontPic_Leader_Maylene, 0x800),
    TRAINER_SPRITE(LEADER_CRASHER_WAKE, gTrainerFrontPic_Leader_Crasher_Wake, 0x800),
    TRAINER_SPRITE(LEADER_FANTINA, gTrainerFrontPic_Leader_Fantina, 0x800),
    TRAINER_SPRITE(LEADER_BYRON, gTrainerFrontPic_Leader_Byron, 0x800),
    TRAINER_SPRITE(LEADER_CANDICE, gTrainerFrontPic_Leader_Candice, 0x800),
    TRAINER_SPRITE(LEADER_VOLKNER, gTrainerFrontPic_Leader_Volkner, 0x800),
    TRAINER_SPRITE(RED_RBY, gTrainerFrontPic_Red_RBY, 0x800),
    TRAINER_SPRITE(GOLD, gTrainerFrontPic_Gold, 0x800),
    TRAINER_SPRITE(ORAS_BRENDAN, gTrainerFrontPic_BrendanORAS, 0x800),
    TRAINER_SPRITE(ORAS_MAY, gTrainerFrontPic_MayORAS, 0x800),
    TRAINER_SPRITE(GREEN, gTrainerFrontPic_Green, 0x800),
    TRAINER_SPRITE(DAWN_PT, gTrainerFrontPic_DawnPt, 0x800),
// ROWENEWTRN-END fronttbl
// CHARMODE-SPRITES-START donor-front-sprites (generated by tools/character_mode/import_sprites.py)
    TRAINER_SPRITE(GARY, gTrainerFrontPic_Gary, 0x800),
    TRAINER_SPRITE(RITCHIE, gTrainerFrontPic_Ritchie, 0x800),
    TRAINER_SPRITE(TRACEY, gTrainerFrontPic_Tracey, 0x800),
    TRAINER_SPRITE(JESSIE, gTrainerFrontPic_Jessie, 0x800),
    TRAINER_SPRITE(JAMES, gTrainerFrontPic_James, 0x800),
    TRAINER_SPRITE(LYRA, gTrainerFrontPic_Lyra, 0x800),
    TRAINER_SPRITE(SILVER, gTrainerFrontPic_Silver, 0x800),
    TRAINER_SPRITE(LIZA, gTrainerFrontPic_Liza, 0x800),
    TRAINER_SPRITE(DREW, gTrainerFrontPic_Drew, 0x800),
    TRAINER_SPRITE(AARON, gTrainerFrontPic_Aaron, 0x800),
    TRAINER_SPRITE(FLINT, gTrainerFrontPic_Flint, 0x800),
    TRAINER_SPRITE(NATE, gTrainerFrontPic_Nate, 0x800),
    TRAINER_SPRITE(ROSA, gTrainerFrontPic_Rosa, 0x800),
    TRAINER_SPRITE(ALDER, gTrainerFrontPic_Alder, 0x800),
    TRAINER_SPRITE(IRIS, gTrainerFrontPic_Iris, 0x800),
    TRAINER_SPRITE(SHAUNTAL, gTrainerFrontPic_Shauntal, 0x800),
    TRAINER_SPRITE(MARSHAL, gTrainerFrontPic_Marshal, 0x800),
    TRAINER_SPRITE(GRIMSLEY, gTrainerFrontPic_Grimsley, 0x800),
    TRAINER_SPRITE(CAITLIN, gTrainerFrontPic_Caitlin, 0x800),
    TRAINER_SPRITE(CILAN, gTrainerFrontPic_Cilan, 0x800),
    TRAINER_SPRITE(CHILI, gTrainerFrontPic_Chili, 0x800),
    TRAINER_SPRITE(CRESS, gTrainerFrontPic_Cress, 0x800),
    TRAINER_SPRITE(LENORA, gTrainerFrontPic_Lenora, 0x800),
    TRAINER_SPRITE(BURGH, gTrainerFrontPic_Burgh, 0x800),
    TRAINER_SPRITE(ELESA, gTrainerFrontPic_Elesa, 0x800),
    TRAINER_SPRITE(CLAY, gTrainerFrontPic_Clay, 0x800),
    TRAINER_SPRITE(SKYLA, gTrainerFrontPic_Skyla, 0x800),
    TRAINER_SPRITE(BRYCEN, gTrainerFrontPic_Brycen, 0x800),
    TRAINER_SPRITE(DRAYDEN, gTrainerFrontPic_Drayden, 0x800),
    TRAINER_SPRITE(CHEREN, gTrainerFrontPic_Cheren, 0x800),
    TRAINER_SPRITE(ROXIE, gTrainerFrontPic_Roxie, 0x800),
    TRAINER_SPRITE(MARLON, gTrainerFrontPic_Marlon, 0x800),
    TRAINER_SPRITE(BIANCA, gTrainerFrontPic_Bianca, 0x800),
    TRAINER_SPRITE(HUGH, gTrainerFrontPic_Hugh, 0x800),
    TRAINER_SPRITE(N, gTrainerFrontPic_N, 0x800),
    TRAINER_SPRITE(GHETSIS, gTrainerFrontPic_Ghetsis, 0x800),
    TRAINER_SPRITE(COLRESS, gTrainerFrontPic_Colress, 0x800),
    TRAINER_SPRITE(CALEM, gTrainerFrontPic_Calem, 0x800),
    TRAINER_SPRITE(SERENA, gTrainerFrontPic_Serena, 0x800),
    TRAINER_SPRITE(DIANTHA, gTrainerFrontPic_Diantha, 0x800),
    TRAINER_SPRITE(MALVA, gTrainerFrontPic_Malva, 0x800),
    TRAINER_SPRITE(SIEBOLD, gTrainerFrontPic_Siebold, 0x800),
    TRAINER_SPRITE(WIKSTROM, gTrainerFrontPic_Wikstrom, 0x800),
    TRAINER_SPRITE(DRASNA, gTrainerFrontPic_Drasna, 0x800),
    TRAINER_SPRITE(VIOLA, gTrainerFrontPic_Viola, 0x800),
    TRAINER_SPRITE(GRANT, gTrainerFrontPic_Grant, 0x800),
    TRAINER_SPRITE(KORRINA, gTrainerFrontPic_Korrina, 0x800),
    TRAINER_SPRITE(RAMOS, gTrainerFrontPic_Ramos, 0x800),
    TRAINER_SPRITE(CLEMONT, gTrainerFrontPic_Clemont, 0x800),
    TRAINER_SPRITE(VALERIE, gTrainerFrontPic_Valerie, 0x800),
    TRAINER_SPRITE(OLYMPIA, gTrainerFrontPic_Olympia, 0x800),
    TRAINER_SPRITE(WULFRIC, gTrainerFrontPic_Wulfric, 0x800),
    TRAINER_SPRITE(SHAUNA, gTrainerFrontPic_Shauna, 0x800),
    TRAINER_SPRITE(LYSANDRE, gTrainerFrontPic_Lysandre, 0x800),
    TRAINER_SPRITE(ELIO, gTrainerFrontPic_Elio, 0x800),
    TRAINER_SPRITE(SELENE, gTrainerFrontPic_Selene, 0x800),
    TRAINER_SPRITE(KUKUI, gTrainerFrontPic_Kukui, 0x800),
    TRAINER_SPRITE(HAU, gTrainerFrontPic_Hau, 0x800),
    TRAINER_SPRITE(MOLAYNE, gTrainerFrontPic_Molayne, 0x800),
    TRAINER_SPRITE(KAHILI, gTrainerFrontPic_Kahili, 0x800),
    TRAINER_SPRITE(ACEROLA, gTrainerFrontPic_Acerola, 0x800),
    TRAINER_SPRITE(HALA, gTrainerFrontPic_Hala, 0x800),
    TRAINER_SPRITE(OLIVIA, gTrainerFrontPic_Olivia, 0x800),
    TRAINER_SPRITE(NANU, gTrainerFrontPic_Nanu, 0x800),
    TRAINER_SPRITE(HAPU, gTrainerFrontPic_Hapu, 0x800),
    TRAINER_SPRITE(GLADION, gTrainerFrontPic_Gladion, 0x800),
    TRAINER_SPRITE(LILLIE, gTrainerFrontPic_Lillie, 0x800),
    TRAINER_SPRITE(KIAWE, gTrainerFrontPic_Kiawe, 0x800),
    TRAINER_SPRITE(LANA, gTrainerFrontPic_Lana, 0x800),
    TRAINER_SPRITE(MALLOW, gTrainerFrontPic_Mallow, 0x800),
    TRAINER_SPRITE(SOPHOCLES, gTrainerFrontPic_Sophocles, 0x800),
    TRAINER_SPRITE(LEON, gTrainerFrontPic_Leon, 0x800),
    TRAINER_SPRITE(MILO, gTrainerFrontPic_Milo, 0x800),
    TRAINER_SPRITE(NESSA, gTrainerFrontPic_Nessa, 0x800),
    TRAINER_SPRITE(KABU, gTrainerFrontPic_Kabu, 0x800),
    TRAINER_SPRITE(BEA, gTrainerFrontPic_Bea, 0x800),
    TRAINER_SPRITE(ALLISTER, gTrainerFrontPic_Allister, 0x800),
    TRAINER_SPRITE(OPAL, gTrainerFrontPic_Opal, 0x800),
    TRAINER_SPRITE(GORDIE, gTrainerFrontPic_Gordie, 0x800),
    TRAINER_SPRITE(MELONY, gTrainerFrontPic_Melony, 0x800),
    TRAINER_SPRITE(PIERS, gTrainerFrontPic_Piers, 0x800),
    TRAINER_SPRITE(RAIHAN, gTrainerFrontPic_Raihan, 0x800),
    TRAINER_SPRITE(HOP, gTrainerFrontPic_Hop, 0x800),
    TRAINER_SPRITE(BEDE, gTrainerFrontPic_Bede, 0x800),
    TRAINER_SPRITE(MARNIE, gTrainerFrontPic_Marnie, 0x800),
    TRAINER_SPRITE(GEETA, gTrainerFrontPic_Geeta, 0x800),
    TRAINER_SPRITE(NEMONA, gTrainerFrontPic_Nemona, 0x800),
    TRAINER_SPRITE(RIKA, gTrainerFrontPic_Rika, 0x800),
    TRAINER_SPRITE(POPPY, gTrainerFrontPic_Poppy, 0x800),
    TRAINER_SPRITE(HASSEL, gTrainerFrontPic_Hassel, 0x800),
    TRAINER_SPRITE(KATY, gTrainerFrontPic_Katy, 0x800),
    TRAINER_SPRITE(BRASSIUS, gTrainerFrontPic_Brassius, 0x800),
    TRAINER_SPRITE(IONO, gTrainerFrontPic_Iono, 0x800),
    TRAINER_SPRITE(KOFU, gTrainerFrontPic_Kofu, 0x800),
    TRAINER_SPRITE(LARRY, gTrainerFrontPic_Larry, 0x800),
    TRAINER_SPRITE(RYME, gTrainerFrontPic_Ryme, 0x800),
    TRAINER_SPRITE(TULIP, gTrainerFrontPic_Tulip, 0x800),
    TRAINER_SPRITE(GRUSHA, gTrainerFrontPic_Grusha, 0x800),
    TRAINER_SPRITE(ARVEN, gTrainerFrontPic_Arven, 0x800),
    TRAINER_SPRITE(PENNY, gTrainerFrontPic_Penny, 0x800),
    TRAINER_SPRITE(VICTOR, gTrainerFrontPic_Victor, 0x800),
    TRAINER_SPRITE(GLORIA, gTrainerFrontPic_Gloria, 0x800),
    TRAINER_SPRITE(OAK, gTrainerFrontPic_Oak, 0x800),
    TRAINER_SPRITE(ARGENTA, gTrainerFrontPic_Argenta, 0x800),
    TRAINER_SPRITE(DAHLIA, gTrainerFrontPic_Dahlia, 0x800),
    TRAINER_SPRITE(DARACH, gTrainerFrontPic_Darach, 0x800),
    TRAINER_SPRITE(PALMER, gTrainerFrontPic_Palmer, 0x800),
    TRAINER_SPRITE(THORTON, gTrainerFrontPic_Thorton, 0x800),
// CHARMODE-SPRITES-END donor-front-sprites
};

const struct SpritePalette gTrainerFrontPicPaletteTable[] =
{
    TRAINER_PAL(HIKER, gTrainerPalette_Hiker),
    TRAINER_PAL(AQUA_GRUNT_M, gTrainerPalette_AquaGruntM),
    TRAINER_PAL(POKEMON_BREEDER_F, gTrainerPalette_PokemonBreederF),
    TRAINER_PAL(COOLTRAINER_M, gTrainerPalette_CoolTrainerM),
    TRAINER_PAL(BIRD_KEEPER, gTrainerPalette_BirdKeeper),
    TRAINER_PAL(COLLECTOR, gTrainerPalette_Collector),
    TRAINER_PAL(AQUA_GRUNT_F, gTrainerPalette_AquaGruntF),
    TRAINER_PAL(SWIMMER_M, gTrainerPalette_SwimmerM),
    TRAINER_PAL(MAGMA_GRUNT_M, gTrainerPalette_MagmaGruntM),
    TRAINER_PAL(EXPERT_M, gTrainerPalette_ExpertM),
    TRAINER_PAL(AQUA_ADMIN_M, gTrainerPalette_AquaAdminM),
    TRAINER_PAL(BLACK_BELT, gTrainerPalette_BlackBelt),
    TRAINER_PAL(AQUA_ADMIN_F, gTrainerPalette_AquaAdminF),
    TRAINER_PAL(AQUA_LEADER_ARCHIE, gTrainerPalette_AquaLeaderArchie),
    TRAINER_PAL(HEX_MANIAC, gTrainerPalette_HexManiac),
    TRAINER_PAL(AROMA_LADY, gTrainerPalette_AromaLady),
    TRAINER_PAL(RUIN_MANIAC, gTrainerPalette_RuinManiac),
    TRAINER_PAL(INTERVIEWER, gTrainerPalette_Interviewer),
    TRAINER_PAL(TUBER_F, gTrainerPalette_TuberF),
    TRAINER_PAL(TUBER_M, gTrainerPalette_TuberM),
    TRAINER_PAL(COOLTRAINER_F, gTrainerPalette_CoolTrainerF),
    TRAINER_PAL(LADY, gTrainerPalette_Lady),
    TRAINER_PAL(BEAUTY, gTrainerPalette_Beauty),
    TRAINER_PAL(RICH_BOY, gTrainerPalette_RichBoy),
    TRAINER_PAL(EXPERT_F, gTrainerPalette_ExpertF),
    TRAINER_PAL(POKEMANIAC, gTrainerPalette_Pokemaniac),
    TRAINER_PAL(MAGMA_GRUNT_F, gTrainerPalette_MagmaGruntF),
    TRAINER_PAL(GUITARIST, gTrainerPalette_Guitarist),
    TRAINER_PAL(KINDLER, gTrainerPalette_Kindler),
    TRAINER_PAL(CAMPER, gTrainerPalette_Camper),
    TRAINER_PAL(PICNICKER, gTrainerPalette_Picnicker),
    TRAINER_PAL(BUG_MANIAC, gTrainerPalette_BugManiac),
    TRAINER_PAL(POKEMON_BREEDER_M, gTrainerPalette_PokemonBreederM),
    TRAINER_PAL(PSYCHIC_M, gTrainerPalette_PsychicM),
    TRAINER_PAL(PSYCHIC_F, gTrainerPalette_PsychicF),
    TRAINER_PAL(GENTLEMAN, gTrainerPalette_Gentleman),
    TRAINER_PAL(ELITE_FOUR_SIDNEY, gTrainerPalette_EliteFourSidney),
    TRAINER_PAL(ELITE_FOUR_PHOEBE, gTrainerPalette_EliteFourPhoebe),
    TRAINER_PAL(ELITE_FOUR_GLACIA, gTrainerPalette_EliteFourGlacia),
    TRAINER_PAL(ELITE_FOUR_DRAKE, gTrainerPalette_EliteFourDrake),
    TRAINER_PAL(LEADER_ROXANNE, gTrainerPalette_LeaderRoxanne),
    TRAINER_PAL(LEADER_BRAWLY, gTrainerPalette_LeaderBrawly),
    TRAINER_PAL(LEADER_WATTSON, gTrainerPalette_LeaderWattson),
    TRAINER_PAL(LEADER_FLANNERY, gTrainerPalette_LeaderFlannery),
    TRAINER_PAL(LEADER_NORMAN, gTrainerPalette_LeaderNorman),
    TRAINER_PAL(LEADER_WINONA, gTrainerPalette_LeaderWinona),
    TRAINER_PAL(LEADER_TATE_AND_LIZA, gTrainerPalette_LeaderTateAndLiza),
    TRAINER_PAL(LEADER_JUAN, gTrainerPalette_LeaderJuan),
    TRAINER_PAL(SCHOOL_KID_M, gTrainerPalette_SchoolKidM),
    TRAINER_PAL(SCHOOL_KID_F, gTrainerPalette_SchoolKidF),
    TRAINER_PAL(SR_AND_JR, gTrainerPalette_SrAndJr),
    TRAINER_PAL(POKEFAN_M, gTrainerPalette_PokefanM),
    TRAINER_PAL(POKEFAN_F, gTrainerPalette_PokefanF),
    TRAINER_PAL(YOUNGSTER, gTrainerPalette_Youngster),
    TRAINER_PAL(CHAMPION_WALLACE, gTrainerPalette_ChampionWallace),
    TRAINER_PAL(FISHERMAN, gTrainerPalette_Fisherman),
    TRAINER_PAL(CYCLING_TRIATHLETE_M, gTrainerPalette_CyclingTriathleteM),
    TRAINER_PAL(CYCLING_TRIATHLETE_F, gTrainerPalette_CyclingTriathleteF),
    TRAINER_PAL(RUNNING_TRIATHLETE_M, gTrainerPalette_RunningTriathleteM),
    TRAINER_PAL(RUNNING_TRIATHLETE_F, gTrainerPalette_RunningTriathleteF),
    TRAINER_PAL(SWIMMING_TRIATHLETE_M, gTrainerPalette_SwimmingTriathleteM),
    TRAINER_PAL(SWIMMING_TRIATHLETE_F, gTrainerPalette_SwimmingTriathleteF),
    TRAINER_PAL(DRAGON_TAMER, gTrainerPalette_DragonTamer),
    TRAINER_PAL(NINJA_BOY, gTrainerPalette_NinjaBoy),
    TRAINER_PAL(BATTLE_GIRL, gTrainerPalette_BattleGirl),
    TRAINER_PAL(PARASOL_LADY, gTrainerPalette_ParasolLady),
    TRAINER_PAL(SWIMMER_F, gTrainerPalette_SwimmerF),
    TRAINER_PAL(TWINS, gTrainerPalette_Twins),
    TRAINER_PAL(SAILOR, gTrainerPalette_Sailor),
    TRAINER_PAL(MAGMA_ADMIN, gTrainerPalette_MagmaAdmin),
    TRAINER_PAL(WALLY, gTrainerPalette_Wally),
    TRAINER_PAL(BRENDAN, gTrainerPalette_Brendan),
    TRAINER_PAL(MAY, gTrainerPalette_May),
    TRAINER_PAL(BUG_CATCHER, gTrainerPalette_BugCatcher),
    TRAINER_PAL(POKEMON_RANGER_M, gTrainerPalette_PokemonRangerM),
    TRAINER_PAL(POKEMON_RANGER_F, gTrainerPalette_PokemonRangerF),
    TRAINER_PAL(MAGMA_LEADER_MAXIE, gTrainerPalette_MagmaLeaderMaxie),
    TRAINER_PAL(LASS, gTrainerPalette_Lass),
    TRAINER_PAL(YOUNG_COUPLE, gTrainerPalette_YoungCouple),
    TRAINER_PAL(OLD_COUPLE, gTrainerPalette_OldCouple),
    TRAINER_PAL(SIS_AND_BRO, gTrainerPalette_SisAndBro),
    TRAINER_PAL(STEVEN, gTrainerPalette_Steven),
    TRAINER_PAL(SALON_MAIDEN_ANABEL, gTrainerPalette_SalonMaidenAnabel),
    TRAINER_PAL(DOME_ACE_TUCKER, gTrainerPalette_DomeAceTucker),
    TRAINER_PAL(PALACE_MAVEN_SPENSER, gTrainerPalette_PalaceMavenSpenser),
    TRAINER_PAL(ARENA_TYCOON_GRETA, gTrainerPalette_ArenaTycoonGreta),
    TRAINER_PAL(FACTORY_HEAD_NOLAND, gTrainerPalette_FactoryHeadNoland),
    TRAINER_PAL(PIKE_QUEEN_LUCY, gTrainerPalette_PikeQueenLucy),
    TRAINER_PAL(PYRAMID_KING_BRANDON, gTrainerPalette_PyramidKingBrandon),
    TRAINER_PAL(RED, gTrainerPalette_Red),
    TRAINER_PAL(LEAF, gTrainerPalette_Leaf),
    TRAINER_PAL(RS_BRENDAN, gTrainerPalette_RubySapphireBrendan),
    TRAINER_PAL(RS_MAY, gTrainerPalette_RubySapphireMay),
	TRAINER_PAL(CRUSH_KIN, gTrainerPalette_CrushKin),
	TRAINER_PAL(NURSE, gTrainerPalette_BlueNurse),
	TRAINER_PAL(HILBERT, gTrainerPalette_Hilbert),
// CHARMODE-SPRITES-START front-pals (generated by tools/character_mode/import_sprites.py)
	TRAINER_PAL(BLUE, gTrainerPalette_CmBlue),
	TRAINER_PAL(LANCE, gTrainerPalette_CmLance),
	TRAINER_PAL(LORELEI, gTrainerPalette_CmLorelei),
	TRAINER_PAL(BRUNO, gTrainerPalette_CmBruno),
	TRAINER_PAL(AGATHA, gTrainerPalette_CmAgatha),
	TRAINER_PAL(KOGA, gTrainerPalette_CmKoga),
	TRAINER_PAL(BROCK, gTrainerPalette_CmBrock),
	TRAINER_PAL(MISTY, gTrainerPalette_CmMisty),
	TRAINER_PAL(LT_SURGE, gTrainerPalette_CmLtSurge),
	TRAINER_PAL(ERIKA, gTrainerPalette_CmErika),
	TRAINER_PAL(SABRINA, gTrainerPalette_CmSabrina),
	TRAINER_PAL(BLAINE, gTrainerPalette_CmBlaine),
	TRAINER_PAL(GIOVANNI, gTrainerPalette_CmGiovanni),
	TRAINER_PAL(LUCAS, gTrainerPalette_CmLucas),
	TRAINER_PAL(DAWN, gTrainerPalette_CmDawn),
	TRAINER_PAL(BARRY, gTrainerPalette_CmBarry),
	TRAINER_PAL(CYNTHIA, gTrainerPalette_CmCynthia),
	TRAINER_PAL(ROARK, gTrainerPalette_CmRoark),
	TRAINER_PAL(GARDENIA, gTrainerPalette_CmGardenia),
	TRAINER_PAL(MAYLENE, gTrainerPalette_CmMaylene),
	TRAINER_PAL(CRASHER_WAKE, gTrainerPalette_CmCrasherWake),
	TRAINER_PAL(FANTINA, gTrainerPalette_CmFantina),
	TRAINER_PAL(BYRON, gTrainerPalette_CmByron),
	TRAINER_PAL(CANDICE, gTrainerPalette_CmCandice),
	TRAINER_PAL(VOLKNER, gTrainerPalette_CmVolkner),
	TRAINER_PAL(BERTHA, gTrainerPalette_CmBertha),
	TRAINER_PAL(LUCIAN, gTrainerPalette_CmLucian),
	TRAINER_PAL(CYRUS, gTrainerPalette_CmCyrus),
	TRAINER_PAL(MARS, gTrainerPalette_CmMars),
	TRAINER_PAL(JUPITER, gTrainerPalette_CmJupiter),
	TRAINER_PAL(SATURN, gTrainerPalette_CmSaturn),
	TRAINER_PAL(ETHAN, gTrainerPalette_CmEthan),
	TRAINER_PAL(KRIS, gTrainerPalette_CmKris),
	TRAINER_PAL(FALKNER, gTrainerPalette_CmFalkner),
	TRAINER_PAL(BUGSY, gTrainerPalette_CmBugsy),
	TRAINER_PAL(WHITNEY, gTrainerPalette_CmWhitney),
	TRAINER_PAL(MORTY, gTrainerPalette_CmMorty),
	TRAINER_PAL(CHUCK, gTrainerPalette_CmChuck),
	TRAINER_PAL(JASMINE, gTrainerPalette_CmJasmine),
	TRAINER_PAL(PRYCE, gTrainerPalette_CmPryce),
	TRAINER_PAL(CLAIR, gTrainerPalette_CmClair),
	TRAINER_PAL(JANINE, gTrainerPalette_CmJanine),
	TRAINER_PAL(WILL, gTrainerPalette_CmWill),
	TRAINER_PAL(KAREN, gTrainerPalette_CmKaren),
	TRAINER_PAL(ARCHER, gTrainerPalette_CmArcher),
	TRAINER_PAL(ARIANA, gTrainerPalette_CmAriana),
// CHARMODE-SPRITES-END front-pals
// ROWENEWTRN-START paltbl (generated by tools/character_mode/port_2x_new_trainers.py)
    TRAINER_PAL(HILDA, gTrainerPalette_Hilda),
    TRAINER_PAL(PAINTER, gTrainerPalette_Painter),
    TRAINER_PAL(ROCKET_GRUNT_M, gTrainerPalette_Rocket_Grunt_M),
    TRAINER_PAL(ROCKET_GRUNT_F, gTrainerPalette_Rocket_Grunt_F),
    TRAINER_PAL(SCIENTIST, gTrainerPalette_Scientist),
    TRAINER_PAL(JUGGLER, gTrainerPalette_Juggler),
    TRAINER_PAL(COOL_COUPLE, gTrainerPalette_Cool_Couple),
    TRAINER_PAL(TAMER, gTrainerPalette_Tamer),
    TRAINER_PAL(CRUSH_GIRL, gTrainerPalette_Crush_Girl),
    TRAINER_PAL(POKEMON_BREEDER, gTrainerPalette_Pokemon_Breeder),
    TRAINER_PAL(LEADER_BROCK, gTrainerPalette_Leader_Brock),
    TRAINER_PAL(LEADER_MISTY, gTrainerPalette_Leader_Misty),
    TRAINER_PAL(LEADER_LT_SURGE, gTrainerPalette_Leader_Lt_Surge),
    TRAINER_PAL(LEADER_ERIKA, gTrainerPalette_Leader_Erika),
    TRAINER_PAL(LEADER_KOGA, gTrainerPalette_Leader_Koga),
    TRAINER_PAL(LEADER_BLAINE, gTrainerPalette_Leader_Blaine),
    TRAINER_PAL(LEADER_SABRINA, gTrainerPalette_Leader_Sabrina),
    TRAINER_PAL(LEADER_GIOVANNI, gTrainerPalette_Leader_Giovanni),
    TRAINER_PAL(ELITE_FOUR_LORELEI, gTrainerPalette_Elite_Four_Lorelei),
    TRAINER_PAL(ELITE_FOUR_BRUNO, gTrainerPalette_Elite_Four_Bruno),
    TRAINER_PAL(ELITE_FOUR_AGATHA, gTrainerPalette_Elite_Four_Agatha),
    TRAINER_PAL(ELITE_FOUR_LANCE, gTrainerPalette_Elite_Four_Lance),
    TRAINER_PAL(CHAMPION_RIVAL, gTrainerPalette_Champion_Rival),
    TRAINER_PAL(LEADER_FALKNER, gTrainerPalette_Leader_Falkner),
    TRAINER_PAL(LEADER_BUGSY, gTrainerPalette_Leader_Bugsy),
    TRAINER_PAL(LEADER_WHITNEY, gTrainerPalette_Leader_Whitney),
    TRAINER_PAL(LEADER_MORTY, gTrainerPalette_Leader_Morty),
    TRAINER_PAL(LEADER_CHUCK, gTrainerPalette_Leader_Chuck),
    TRAINER_PAL(LEADER_JASMINE, gTrainerPalette_Leader_Jasmine),
    TRAINER_PAL(LEADER_PRYCE, gTrainerPalette_Leader_Pryce),
    TRAINER_PAL(LEADER_CLAIR, gTrainerPalette_Leader_Clair),
    TRAINER_PAL(LEADER_ROARK, gTrainerPalette_Leader_Roark),
    TRAINER_PAL(LEADER_GARDENIA, gTrainerPalette_Leader_Gardenia),
    TRAINER_PAL(LEADER_MAYLENE, gTrainerPalette_Leader_Maylene),
    TRAINER_PAL(LEADER_CRASHER_WAKE, gTrainerPalette_Leader_Crasher_Wake),
    TRAINER_PAL(LEADER_FANTINA, gTrainerPalette_Leader_Fantina),
    TRAINER_PAL(LEADER_BYRON, gTrainerPalette_Leader_Byron),
    TRAINER_PAL(LEADER_CANDICE, gTrainerPalette_Leader_Candice),
    TRAINER_PAL(LEADER_VOLKNER, gTrainerPalette_Leader_Volkner),
    TRAINER_PAL(RED_RBY, gTrainerPalette_Red_RBY),
    TRAINER_PAL(GOLD, gTrainerPalette_Gold),
    TRAINER_PAL(ORAS_BRENDAN, gTrainerPalette_BrendanORAS),
    TRAINER_PAL(ORAS_MAY, gTrainerPalette_MayORAS),
    TRAINER_PAL(GREEN, gTrainerPalette_Green),
    TRAINER_PAL(DAWN_PT, gTrainerPalette_DawnPt),
// ROWENEWTRN-END paltbl
// CHARMODE-SPRITES-START donor-front-pals (generated by tools/character_mode/import_sprites.py)
    TRAINER_PAL(GARY, gTrainerPalette_Gary),
    TRAINER_PAL(RITCHIE, gTrainerPalette_Ritchie),
    TRAINER_PAL(TRACEY, gTrainerPalette_Tracey),
    TRAINER_PAL(JESSIE, gTrainerPalette_Jessie),
    TRAINER_PAL(JAMES, gTrainerPalette_James),
    TRAINER_PAL(LYRA, gTrainerPalette_Lyra),
    TRAINER_PAL(SILVER, gTrainerPalette_Silver),
    TRAINER_PAL(LIZA, gTrainerPalette_Liza),
    TRAINER_PAL(DREW, gTrainerPalette_Drew),
    TRAINER_PAL(AARON, gTrainerPalette_Aaron),
    TRAINER_PAL(FLINT, gTrainerPalette_Flint),
    TRAINER_PAL(NATE, gTrainerPalette_Nate),
    TRAINER_PAL(ROSA, gTrainerPalette_Rosa),
    TRAINER_PAL(ALDER, gTrainerPalette_Alder),
    TRAINER_PAL(IRIS, gTrainerPalette_Iris),
    TRAINER_PAL(SHAUNTAL, gTrainerPalette_Shauntal),
    TRAINER_PAL(MARSHAL, gTrainerPalette_Marshal),
    TRAINER_PAL(GRIMSLEY, gTrainerPalette_Grimsley),
    TRAINER_PAL(CAITLIN, gTrainerPalette_Caitlin),
    TRAINER_PAL(CILAN, gTrainerPalette_Cilan),
    TRAINER_PAL(CHILI, gTrainerPalette_Chili),
    TRAINER_PAL(CRESS, gTrainerPalette_Cress),
    TRAINER_PAL(LENORA, gTrainerPalette_Lenora),
    TRAINER_PAL(BURGH, gTrainerPalette_Burgh),
    TRAINER_PAL(ELESA, gTrainerPalette_Elesa),
    TRAINER_PAL(CLAY, gTrainerPalette_Clay),
    TRAINER_PAL(SKYLA, gTrainerPalette_Skyla),
    TRAINER_PAL(BRYCEN, gTrainerPalette_Brycen),
    TRAINER_PAL(DRAYDEN, gTrainerPalette_Drayden),
    TRAINER_PAL(CHEREN, gTrainerPalette_Cheren),
    TRAINER_PAL(ROXIE, gTrainerPalette_Roxie),
    TRAINER_PAL(MARLON, gTrainerPalette_Marlon),
    TRAINER_PAL(BIANCA, gTrainerPalette_Bianca),
    TRAINER_PAL(HUGH, gTrainerPalette_Hugh),
    TRAINER_PAL(N, gTrainerPalette_N),
    TRAINER_PAL(GHETSIS, gTrainerPalette_Ghetsis),
    TRAINER_PAL(COLRESS, gTrainerPalette_Colress),
    TRAINER_PAL(CALEM, gTrainerPalette_Calem),
    TRAINER_PAL(SERENA, gTrainerPalette_Serena),
    TRAINER_PAL(DIANTHA, gTrainerPalette_Diantha),
    TRAINER_PAL(MALVA, gTrainerPalette_Malva),
    TRAINER_PAL(SIEBOLD, gTrainerPalette_Siebold),
    TRAINER_PAL(WIKSTROM, gTrainerPalette_Wikstrom),
    TRAINER_PAL(DRASNA, gTrainerPalette_Drasna),
    TRAINER_PAL(VIOLA, gTrainerPalette_Viola),
    TRAINER_PAL(GRANT, gTrainerPalette_Grant),
    TRAINER_PAL(KORRINA, gTrainerPalette_Korrina),
    TRAINER_PAL(RAMOS, gTrainerPalette_Ramos),
    TRAINER_PAL(CLEMONT, gTrainerPalette_Clemont),
    TRAINER_PAL(VALERIE, gTrainerPalette_Valerie),
    TRAINER_PAL(OLYMPIA, gTrainerPalette_Olympia),
    TRAINER_PAL(WULFRIC, gTrainerPalette_Wulfric),
    TRAINER_PAL(SHAUNA, gTrainerPalette_Shauna),
    TRAINER_PAL(LYSANDRE, gTrainerPalette_Lysandre),
    TRAINER_PAL(ELIO, gTrainerPalette_Elio),
    TRAINER_PAL(SELENE, gTrainerPalette_Selene),
    TRAINER_PAL(KUKUI, gTrainerPalette_Kukui),
    TRAINER_PAL(HAU, gTrainerPalette_Hau),
    TRAINER_PAL(MOLAYNE, gTrainerPalette_Molayne),
    TRAINER_PAL(KAHILI, gTrainerPalette_Kahili),
    TRAINER_PAL(ACEROLA, gTrainerPalette_Acerola),
    TRAINER_PAL(HALA, gTrainerPalette_Hala),
    TRAINER_PAL(OLIVIA, gTrainerPalette_Olivia),
    TRAINER_PAL(NANU, gTrainerPalette_Nanu),
    TRAINER_PAL(HAPU, gTrainerPalette_Hapu),
    TRAINER_PAL(GLADION, gTrainerPalette_Gladion),
    TRAINER_PAL(LILLIE, gTrainerPalette_Lillie),
    TRAINER_PAL(KIAWE, gTrainerPalette_Kiawe),
    TRAINER_PAL(LANA, gTrainerPalette_Lana),
    TRAINER_PAL(MALLOW, gTrainerPalette_Mallow),
    TRAINER_PAL(SOPHOCLES, gTrainerPalette_Sophocles),
    TRAINER_PAL(LEON, gTrainerPalette_Leon),
    TRAINER_PAL(MILO, gTrainerPalette_Milo),
    TRAINER_PAL(NESSA, gTrainerPalette_Nessa),
    TRAINER_PAL(KABU, gTrainerPalette_Kabu),
    TRAINER_PAL(BEA, gTrainerPalette_Bea),
    TRAINER_PAL(ALLISTER, gTrainerPalette_Allister),
    TRAINER_PAL(OPAL, gTrainerPalette_Opal),
    TRAINER_PAL(GORDIE, gTrainerPalette_Gordie),
    TRAINER_PAL(MELONY, gTrainerPalette_Melony),
    TRAINER_PAL(PIERS, gTrainerPalette_Piers),
    TRAINER_PAL(RAIHAN, gTrainerPalette_Raihan),
    TRAINER_PAL(HOP, gTrainerPalette_Hop),
    TRAINER_PAL(BEDE, gTrainerPalette_Bede),
    TRAINER_PAL(MARNIE, gTrainerPalette_Marnie),
    TRAINER_PAL(GEETA, gTrainerPalette_Geeta),
    TRAINER_PAL(NEMONA, gTrainerPalette_Nemona),
    TRAINER_PAL(RIKA, gTrainerPalette_Rika),
    TRAINER_PAL(POPPY, gTrainerPalette_Poppy),
    TRAINER_PAL(HASSEL, gTrainerPalette_Hassel),
    TRAINER_PAL(KATY, gTrainerPalette_Katy),
    TRAINER_PAL(BRASSIUS, gTrainerPalette_Brassius),
    TRAINER_PAL(IONO, gTrainerPalette_Iono),
    TRAINER_PAL(KOFU, gTrainerPalette_Kofu),
    TRAINER_PAL(LARRY, gTrainerPalette_Larry),
    TRAINER_PAL(RYME, gTrainerPalette_Ryme),
    TRAINER_PAL(TULIP, gTrainerPalette_Tulip),
    TRAINER_PAL(GRUSHA, gTrainerPalette_Grusha),
    TRAINER_PAL(ARVEN, gTrainerPalette_Arven),
    TRAINER_PAL(PENNY, gTrainerPalette_Penny),
    TRAINER_PAL(VICTOR, gTrainerPalette_Victor),
    TRAINER_PAL(GLORIA, gTrainerPalette_Gloria),
    TRAINER_PAL(OAK, gTrainerPalette_Oak),
    TRAINER_PAL(ARGENTA, gTrainerPalette_Argenta),
    TRAINER_PAL(DAHLIA, gTrainerPalette_Dahlia),
    TRAINER_PAL(DARACH, gTrainerPalette_Darach),
    TRAINER_PAL(PALMER, gTrainerPalette_Palmer),
    TRAINER_PAL(THORTON, gTrainerPalette_Thorton),
// CHARMODE-SPRITES-END donor-front-pals
};
