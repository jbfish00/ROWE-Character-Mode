...
**Phase 4: COMPLETE AND TESTED IN-GAME.** 16-badge system (Johto leaders =
badges 9-16, Mega Bracelet moved 7 -> 11), Landorus legendary, and the islands
are startable. Verified by starting a new game on One Island (map group 0) and
Eight Island (group 2, brand-new 2.X content): tilesets, NPCs, collision all fine.
...
- **Hardcoded counts vs grown data.** Five instances so far: `gBattleAnims_Moves`
  (the Trick-o-nome crash), `sScrollingSets`, the badge-indexed scaling arrays,
  `NUM_SOFT_CAPS`, and `MAP_GROUP_COUNT[]` in debug.c (216 maps unreachable).
