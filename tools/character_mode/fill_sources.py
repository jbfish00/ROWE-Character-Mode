...
def family_names():
    ...
    _collapse, siblings = erd.form_tables()
    display = erd.species_display_names()

    def walk(const):
        out, stack, seen = set(), [const], set()
        while stack:
            cur = stack.pop()
            if cur in seen:
                continue
    ...
    by_display = {}
    for const, name in display.items():
        by_display.setdefault(name, const)
    return {name: {display.get(c) for c in walk(const) if display.get(c)}
            for name, const in by_display.items()}
